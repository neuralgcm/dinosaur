...
def cumulative_integral_over_sigma(
  ...
  """Approximates cumulative integral of `x` over `sigma` coordinate."""
  if coordinates.layers != x.shape[axis]:
    raise ValueError(
        '`x.shape[axis]` must be equal to `coordinates.layers`;'
        f'got {x.shape[axis]} and {coordinates.layers}.'
...
def centered_difference(
  ...
  """Derivative of `x` with respect to `eta` along specified `axis`.
  ...
  Returns:
    An array `x_eta` with `x_eta.shape[axis] == x.shape[axis] - 1`,
...
def centered_vertical_advection(
    ...
    dx_deta_boundary_values: tuple[Array, Array] | None = None,
) -> jnp.ndarray:
  ...
  if dx_deta_boundary_values is None:
    x_slc_shape = _slice_shape_along_axis(x, axis)
    dx_deta_boundary_values = (
  ...
  x_diff_boundary_top, x_diff_boundary_bot = dx_deta_boundary_values
